export class Card {
  ...
  get fecha_vencimiento(): Date { return this._fecha_vencimiento; }
  get saldo(): number { return this._saldo; }
  get id_usuario(): number { return this._id_usuario; }

  // Setters
  set cvv(valor: string) { this._cvv = valor; }
  set fecha_vencimiento(valor: Date) { this._fecha_vencimiento = valor; }
  set saldo(valor: number) { this._saldo = valor; }

  // Propiedades calculadas
  get ultimosDigitos(): string { return this._numero.slice(-4); }
  get numeroEnmascarado(): string {
    const grupos = this._numero.match(/.{1,4}/g) || [];
    return grupos.map((grupo, index) => index === grupos.length - 1 ? grupo : '****').join(' ');
  }
  get estaVencida(): boolean { return this._fecha_vencimiento < new Date(); }

  tieneSaldoSuficiente(monto: number): boolean { return this._saldo >= monto; }

  static crearDesdeDatos(numero: string, cvv: string, fechaVencimiento: Date, saldo: number, idUsuario: number): Card {
