...
import { RegistrarTarjetaComponent } from '../../../general/registrar-tarjeta/registrar-tarjeta.component';
...
export class PagoComponent implements OnInit {
  ...
  constructor(public carritoService: CarritoService, private cardService: CardService, private compraService: CompraService, private authService: AuthService, private router: Router) {}
  ...
  cargarTarjetasUsuario(): void {
    ...
    this.cardService.obtenerTarjetasUsuario(userId).subscribe({
      ...
      error: (error) => {
        ...
        this.mostrarMensaje(`No se pudieron cargar las tarjetas: ${mensajeError}. Por favor, intente nuevamente.`, 'error');
      }
    });
  }
  ...
  abrirConfirmacionPago(): void {
    ...
    if (!tarjeta.tieneSaldoSuficiente(this.totalCompra())) {
      this.mostrarMensaje(`Saldo insuficiente. Necesita Q ${this.totalCompra().toFixed(2)} pero solo tiene Q ${tarjeta.saldo.toFixed(2)}`, 'error');
      return;
    }
          ...
          this.cardService.reducirSaldo(tarjeta.numero, this.totalCompra()).subscribe({
            next: (respuestaPago) => {
              if (respuestaPago.success) {
                const pedidos = respuestas.map((r, i) => `Pedido #${r.id_pedido}`).join(', ');
                this.mostrarMensaje(`¡Compra realizada exitosamente! ${pedidos}. Total cobrado: Q ${this.totalCompra().toFixed(2)}`, 'success');
                this.carritoService.vaciarCarrito();
                setTimeout(() => {
                  this.router.navigate(['/']);
                }, 4000);
              } else {
                this.mostrarMensaje(`Error en el pago: ${respuestaPago.message}. Las compras fueron registradas pero no se pudo cobrar.`, 'error');
            ...
            error: (errorPago) => {
              ...
              this.mostrarMensaje(`Las compras fueron registradas (${pedidos}) pero hubo un error al cobrar: ${mensajeError}. Por favor contacte con soporte.`, 'error');
            }
          });
        } else {
          this.loading.set(false);
          const fallidas = respuestas.filter(r => !r.exitoso).map(r => r.mensaje).join('; ');
          this.mostrarMensaje(`No se pudieron procesar todas las compras: ${fallidas}`, 'error');
        }
      },
      error: (errorCompra) => {
        console.error('Error al procesar compras:', errorCompra);
        this.loading.set(false);
        const mensajeError = errorCompra?.error?.mensaje || errorCompra?.error?.message || errorCompra?.message || 'Error desconocido';
        this.mostrarMensaje(`Error al procesar las compras: ${mensajeError}. Por favor, intente nuevamente.`, 'error');
      }
    });
  }
  ...
  eliminarTarjeta(): void {
    ...
    this.cardService.eliminarTarjeta(tarjeta.numero).subscribe({
      next: () => {
        this.tarjetasUsuario.update(tarjetas => tarjetas.filter(t => t.numero !== tarjeta.numero));
      ...
      error: (error) => {
        ...
        this.eliminandoTarjeta.set(false);
        const mensajeError = error?.error?.message || error?.message || 'Error desconocido';
        this.mostrarMensaje(`No se pudo eliminar la tarjeta: ${mensajeError}. Por favor, intente nuevamente.`, 'error');
        this.cerrarConfirmacionEliminar();
      }
    });
  }
