...
export class RegistroComponent {
  ...
  passwordsIguales(control: AbstractControl): ValidationErrors | null {
    ...
    if (confirmPassword.hasError('noCoinciden')) confirmPassword.setErrors(null);
    return null;
  }
  ...
  onRegistro(): void {
    ...
    this.userService.crearUsuario(formValue.nombre, formValue.apellido, formValue.email, formValue.password).subscribe({
      next: () => {
      ...
      error: (error) => {
        ...
        this.errorMessage = error.status === 409 ? 'El correo electrónico ya está registrado.' :
                           error.status === 400 ? 'Datos inválidos. Verifique la información ingresada.' :
